...
def build_command(args: argparse.Namespace) -> int:
    ...
    for index, beat in enumerate(board["beats"]):
        ...
        pieces.append(clip(beat["id"], float(beat["start"]), float(beat["end"]), 2 + index, f"beat slot-{esc(beat['slot'])}", body))
...
<style>
html,body{{margin:0;width:1080px;height:1920px;overflow:hidden;background:#111;color:#fff;font-family:Roboto,sans-serif}}*{{box-sizing:border-box}}#root{{position:relative;width:1080px;height:1920px;overflow:hidden}}#root::before{{content:"";position:absolute;inset:0;background:#111;z-index:-2}}video{{position:absolute;inset:0;width:1080px;height:1920px;object-fit:cover;z-index:0}}.clip{{position:absolute;z-index:3}}.beat{{width:660px;top:105px}}.slot-left{{left:38px}}.slot-right{{right:38px}}.slot-hero{{left:50%;transform:translateX(-50%)}}.card-inner{{padding:30px;border:1px solid rgba(210,255,235,.28);border-radius:24px;background:linear-gradient(145deg,rgba(11,29,24,.94),rgba(19,30,29,.86));box-shadow:0 22px 70px rgba(0,0,0,.35)}}small{{color:#8ef2c9;font-weight:800;letter-spacing:2px}}h2{{font-size:52px;line-height:1.08;margin:14px 0 12px}}p{{font-size:27px;line-height:1.42;margin:0;color:#e7eee9}}.tags{{display:flex;gap:10px;margin-top:18px;flex-wrap:wrap}}.tags span{{padding:8px 14px;border:1px solid #bda64f;border-radius:999px;color:#f4d86d;font-size:20px}}.micro{{width:310px;top:720px}}.micro-inner{{padding:18px 22px;border-radius:18px;background:rgba(12,24,21,.92);box-shadow:0 14px 38px rgba(0,0,0,.28)}}.micro strong{{display:block;font-size:28px;margin-top:7px}}.caption{{left:64px;right:64px;bottom:68px;text-align:center;z-index:9}}.caption div{{display:inline-block;max-width:952px;padding:18px 26px;border-radius:18px;background:rgba(0,0,0,.66);font-size:38px;font-weight:750;line-height:1.28;text-shadow:0 2px 3px #000}}
...
<script>document.querySelectorAll('.beat .card-inner,.micro .micro-inner,.caption div').forEach((el)=>{{const host=el.closest('.clip');const start=Number(host.dataset.start)*1000;const total=Number(host.dataset.duration)*1000;const anim=el.animate([{{opacity:0,transform:'translateY(24px) scale(.98)'}},{{opacity:1,transform:'translateY(0) scale(1)',offset:.18}},{{opacity:1,transform:'translateY(0) scale(1)',offset:.88}},{{opacity:0,transform:'translateY(-10px) scale(.99)'}}],{{duration:total,delay:start,easing:'cubic-bezier(.2,0,0,1)',fill:'both',iterations:1}});anim.pause();}});</script></body></html>'''
