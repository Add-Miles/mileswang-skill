...
5. Load only the Miles module needed for the request:
   ...
   - Use [miles-case-learning](../miles-case-learning/SKILL.md) when a supplied product, tool, workflow, repository, creator post, or public case must become an evidence-bounded learning card with its shortest chain and one safe minimal reproduction; do not use it to immediately build the whole product.
   ...
   - Use [miles-prd-framework](../miles-prd-framework/SKILL.md) when a complete PRD must become a lean writing framework, or a half draft needs structure filled against that framework.
